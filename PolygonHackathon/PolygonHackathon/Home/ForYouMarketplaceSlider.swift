...
class ForYouMarketPlaceSlider: UIView {
    ...
    lazy var foryouButton: UIButton = {
        let button = UIButton()
        button.layout(textcolour: .white, backgroundColour: .clear, size: 16, text: "For you", image: nil, cornerRadius: 0)
        return button
    }()
    
    lazy var underlineView: UIView = {
        let view = UIView()
        view.backgroundColor = .polygonPurple
        return view
    }()
    
    lazy var marketplaceButton: UIButton = {
        let button = UIButton()
        button.layout(textcolour: .inactive, backgroundColour: .clear, size: 16, text: "Marketplace", image: nil, cornerRadius: 0)
        return button
    }()
